...
export function recordClinicianAction(input: { patientId: string; action: ClinicianActionType; user: ClinicianUser; note?: string; now?: () => string; store?: ClinicianStore }) {
  ...
  const auditEvent: AuditEvent = { id: `audit-${actionRecord.id}`, actor: "clinician", action: input.action, targetId: input.patientId, createdAt: now };
  ...
  if (input.action === "schedule_follow_up") state.followUps = [...state.followUps, createFollowUp(input.patientId, now)];
  if (input.action === "mark_resolved") state.resolved = true;
  store.save(state);
  return { state, action: actionRecord, auditEvent, followUp: state.followUps.at(-1), patientMessage: input.action === "schedule_follow_up" ? safeFollowUpMessage : undefined };
}
