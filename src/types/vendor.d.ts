...
declare module "vitest" { export function describe(name: string, fn: () => void): void; export function it(name: string, fn: () => void): void; export function expect(value: unknown): { toBe(expected: unknown): void; toEqual(expected: unknown): void; toHaveLength(expected: number): void; toContain(expected: string): void; toThrow(expected?: RegExp): void; }; }
declare module "vitest/config" { export function defineConfig(config: Record<string, unknown>): Record<string, unknown>; }
declare module "zod" {
  export const z: {
    enum<T extends readonly [string, ...string[]]>(values: T): { default(value: T[number]): unknown };
    object<T extends Record<string, unknown>>(shape: T): { superRefine(fn: (env: Record<string, string>, ctx: { addIssue(issue: Record<string, unknown>): void }) => void): { parse(input: unknown): Record<string, string>; }; parse(input: unknown): Record<string, string> };
